...
ipcMain.handle('store-set', async (event, key, value) => {
  console.log('store-set handler called with:', key, value); // Debug log
  // Mask sensitive data in logs
  const logValue = value ? {
    ...value,
    password: '********',
...
ipcMain.handle('chat-message', async (event, message, model, temperature) => {
  ...
  let systemPrompt = "You are a helpful family AI assistant. You help with tasks like homework, scheduling, and general knowledge questions. Keep responses concise and family-friendly.";
  if (memory) {
    systemPrompt += ` Here is what you know about this family: ${memory}`;
  }
...
ipcMain.handle('process-document', async (event, filename, fileData, prompt) => {
    try {
        ...
        // Prepare the prompt
        const fullPrompt = prompt || 'Please analyze this document and provide a summary of its contents.';
        const messages = [
...
ipcMain.handle('summarise-youtube-video', async (event, youtubeUrl) => {
    try {
        const userData = store.get('userData');
        const selectedProvider = store.get('selectedProvider') || 'openai';
        if (!userData || !userData[`${selectedProvider}ApiKey`]) {
        ...
        const summary = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "You are a helpful assistant that summarizes YouTube videos. Provide a concise but comprehensive summary of the video content."
                },
                {
                    role: "user",
...
ipcMain.handle('analyze-image', async (event, fileName, fileData, prompt) => {
  try {
    const userData = store.get('userData');
    const selectedProvider = store.get('selectedProvider') || 'openai';
...
ipcMain.handle('create-image', async (event, prompt) => {
  try {
    const userData = store.get('userData');
    const selectedProvider = store.get('selectedProvider') || 'openai';
    if (!userData || !userData[`${selectedProvider}ApiKey`]) {
