document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function sendMessage(content) {
        ...
        try {
            ...
            const response = await window.electronAPI.chatMessage(actualContent);
            ...
            if (isVoiceInput) {
                ...
                if (!speakerButton.classList.contains('speaking')) {
                    ...
                    const audio = new Audio(URL.createObjectURL(new Blob([await window.electronAPI.textToSpeech(response.message)], { type: 'audio/mpeg' })));
    ...
    if (document.getElementById('summariseYoutubeBtn')) {
        document.getElementById('summariseYoutubeBtn').addEventListener('click', async () => {
            ...
            try {
                const response = await window.electronAPI.summariseYoutubeVideo(url);
    ...
    if (webSearchButton) {
        webSearchButton.addEventListener('click', async () => {
            ...
            try {
                const results = await window.electronAPI.webSearch(userQuery);
                if (!results || results.length === 0) {
                    throw new Error('No results found');
                }
                
                const prompt = buildWebSearchPrompt(userQuery, results);
                const response = await window.electronAPI.chatMessage(prompt, selectedModel);
    ...
    if (imagePromptCreate) {
        imagePromptCreate.addEventListener('click', async () => {
            ...
            try {
                const result = await window.electronAPI.createImage(prompt);
        ...
        try {
            ...
            mediaRecorder.onstop = async () => {
                ...
                try {
                    ...
                    const transcribedText = await window.electronAPI.transcribeAudio(uint8Array);
    ...
    speakerButton.addEventListener('click', async () => {
        ...
        if (messages.length > 0) {
            ...
            try {
                ...
                const audioData = await window.electronAPI.textToSpeech(messageText);
