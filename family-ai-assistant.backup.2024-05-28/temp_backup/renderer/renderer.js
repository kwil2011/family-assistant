    ...
    try {
        ...
        if (selectModelBtn) {
            selectModelBtn.addEventListener('click', async (e) => {
                e.stopPropagation();
                // Get the current provider from userData
                const userData = await window.electronAPI.storeGet('userData');
                const currentProvider = userData.llmProvider || 'openai';
                
                // Set the correct provider tab active
                document.querySelectorAll('.provider-tab').forEach(tab => {
                    if (tab.getAttribute('data-provider') === currentProvider) {
                        tab.classList.add('active');
                    } else {
                ...
                document.querySelectorAll('.model-section').forEach(section => {
                    if (section.getAttribute('data-provider') === currentProvider) {
        ...
        if (imagePromptCreate) {
            imagePromptCreate.addEventListener('click', async () => {
                ...
                try {
                    const chatHistory = JSON.parse(localStorage.getItem('chatHistory') || '[]');
                    const result = await window.electronAPI.createImage(prompt, chatHistory, String(selectedModel));
