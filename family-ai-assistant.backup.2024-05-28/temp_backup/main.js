...
// IPC handlers for environment variables
...
ipcMain.handle('updateEnvironmentVariables', async (event, envVars) => {
    try {
        // Validate keys
        if (!envVars.SERPAPI_API_KEY || !envVars.STORE_ENCRYPTION_KEY) {
            throw new Error('All environment variables are required');
        }
        
        // Check if store encryption key is changing
        if (envVars.STORE_ENCRYPTION_KEY !== process.env.STORE_ENCRYPTION_KEY) {
            // Backup current store data
            const currentStore = store.store;
            
            // Create new store with new encryption key
            const newStore = new Store({
                name: 'config',
                cwd: app.getPath('userData'),
                encryptionKey: envVars.STORE_ENCRYPTION_KEY,
                clearInvalidConfig: true
            });
            
            // Copy data to new store
            Object.entries(currentStore).forEach(([key, value]) => {
                newStore.set(key, value);
            });
            
            // Replace old store with new store
            Object.assign(store, newStore);
        }
        ...
        // Update process.env
        process.env.SERPAPI_API_KEY = envVars.SERPAPI_API_KEY;
        process.env.STORE_ENCRYPTION_KEY = envVars.STORE_ENCRYPTION_KEY;
        
        // Reinitialize SerpAPI client
        if (envVars.SERPAPI_API_KEY) {
            serpApiClient = new SerpApi.GoogleSearch(envVars.SERPAPI_API_KEY);
            console.log('SerpAPI client reinitialized with new key');
        }
